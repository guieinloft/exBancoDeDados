...
select * from filme where titulo like "batman%";
...
select distinct ano, count(idFilme) from filme group by ano order by ano;
-- 6. Exibir nome de diretores e o título de seus filmes.
select d.nome, f.titulo from diretor d, filme f where d.idDiretor = f.idDiretor;
-- 7. Selecionar título e ano de filmes dirigidos por Steven Spielberg. 
-- Ordene pelo ano.
select f.titulo, f.ano from filme f, diretor d where d.idDiretor = f.idDiretor and d.nome = "Steven Spielberg" order by f.ano;
-- 8. Retornar nomes de diretores juntamente com o número de filmes que cada um dirigiu.
select distinct d.nome, count(f.idFilme) from diretor d, filme f where f.idDiretor = d.idDiretor group by d.nome;
...
select distinct d.nome, count(f.idFilme) from diretor d, filme f where f.idDiretor = d.idDiretor group by d.nome having count(f.idFilme) > 2;
-- 10. Para cada diretor, retornar o ano em que teve o primeiro filme lançado.
select distinct d.nome, min(f.ano) from diretor d, filme f where f.idDiretor = d.idDiretor group by d.nome;
